...
function displayData(data) {
    ...
    localStorage.setItem("jaff0021", JSON.stringify(data));
    console.log(data);
    
    
//    var myScoreData = JSON.parse(localStorage.getItem("jaff0021"));
//    console.log("From LS: ");
//    console.log(myScoreData);
    ...
    data.scores.forEach(function(value){
        ...
        value.games.forEach(function (item){ //item equals "data.scores[x]" where x is the loop time
...
function setupStandings(data){
    for(let i = 0; i < team.length; i++)
        {
            team[i]={name: data.teams[i].name, wins:0, loses: 0, ties: 0};
        }
}
